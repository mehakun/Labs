...
class GOST34112012_my(PEP247):
    ...
    def __init__(self, data=b"", digest_size=64, rounds_amount=12):
        """
    ...
    def copy(self):
        return GOST34112012(copy(self.data), self.digest_size, self.rounds_amount)
    ...
    def digest(self):
        ...
        for i in xrange(0, len(data) // BLOCKSIZE * BLOCKSIZE, BLOCKSIZE):
            ...
            hsh = g(n, hsh, block, self.rounds_amount)
        ...
        hsh = g(n, hsh, data[-BLOCKSIZE:], self.rounds_amount)
        n += padblock_size
        chk = add512bit(chk, data[-BLOCKSIZE:])
        hsh = g(0, hsh, pack("<Q", n) + 56 * b"\x00", self.rounds_amount)
        hsh = g(0, hsh, chk, self.rounds_amount)
        return hsh[-self._digest_size:]


def create_random_string(size = 42):
...
def pop_count(a):
    return bin(int.from_bytes(bytes(a), byteorder='big')).count('1')

def main():
    max_rounds_amount = 13
    sample = bytes(create_random_string(), 'ascii')
    byte = bytearray(sample)
    byte[7] = byte[7] ^ (1 << 1)
    xored_sample = bytes(byte)
    diff1 = []
    diff2 = []
    rounds = list(range(1, max_rounds_amount))

    for rounds_amount in rounds:
        hasher1 = GOST34112012_my(sample, digest_size=32, rounds_amount=rounds_amount)
        hasher2 = GOST34112012_my(xored_sample, digest_size=32, rounds_amount=rounds_amount)

        hash1 = hasher1.digest()
        hash2 = hasher2.digest()

        diff1.append(pop_count(xor_bytes(sample, hash1)))
        diff2.append(pop_count(xor_bytes(xored_sample, hash2)))


    print(f'pop count list for unchanged string:\n{diff1}')
    print(f'pop count list for changed string:\n{diff2}')

    plt.xlabel('amount of rounds')
    plt.ylabel('pop_count(str ^ hash(str))')
    plt.plot(rounds, diff1, color='b')
    plt.scatter(rounds, diff1, color='b')
    plt.plot(rounds, diff2, color='r')
    plt.scatter(rounds, diff2, color='r')
    plt.show()
main()